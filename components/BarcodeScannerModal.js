...
function CloseBar({ onClose, label }) {
  ...
  return (
    <View style={{ position: "absolute", top: 50, left: 16, right: 16, flexDirection: "row", alignItems: "center", justifyContent: "space-between" }}>
      ...
      <Pressable onPress={onClose} accessibilityRole="button" accessibilityLabel={t("barcodeScanner.closeScanner")} style={{ backgroundColor: "rgba(0,0,0,0.5)", borderRadius: 999, width: 40, height: 40, alignItems: "center", justifyContent: "center" }}>
...
function ScannerBody({ onScanned, onClose }) {
  ...
  if (!permission) {
    return <View style={{ flex: 1, backgroundColor: "#07120b" }}><CloseBar onClose={onClose} label={t("barcodeScanner.scanPacket")} /></View>;
  }
  ...
  if (!permission.granted) {
    return (
      <View style={{ flex: 1, backgroundColor: "#07120b", alignItems: "center", justifyContent: "center", padding: 28 }}>
        ...
        <Text style={{ color: "#ffffff", fontSize: 16, fontWeight: "900", textAlign: "center" }}>{t("barcodeScanner.cameraNeededTitle")}</Text>
    ...
    <View style={{ flex: 1, backgroundColor: "#000000" }}>
      ...
      <View pointerEvents="none" style={{ position: "absolute", top: 0, left: 0, right: 0, bottom: 0, alignItems: "center", justifyContent: "center" }}>
        ...
        <Text style={{ color: "#ffffff", fontSize: 13, fontWeight: "800", marginTop: 16, backgroundColor: "rgba(0,0,0,0.5)", paddingHorizontal: 12, paddingVertical: 6, borderRadius: 999 }}>
          {t("barcodeScanner.pointAtCode")}
        </Text>
      </View>
      <CloseBar onClose={onClose} label={t("barcodeScanner.scanPacket")} />
    </View>
  );
}
    ...
    <Modal visible={visible} animationType="slide" onRequestClose={onClose}>
      <NativeModuleGuard
        fallback={
          <View style={{ flex: 1, backgroundColor: "#07120b", alignItems: "center", justifyContent: "center", padding: 28 }}>
            <Text style={{ fontSize: 44, marginBottom: 14 }}>📷</Text>
            <Text style={{ color: "#ffffff", fontSize: 16, fontWeight: "900", textAlign: "center" }}>{t("barcodeScanner.needsNewBuildTitle")}</Text>
            <Text style={{ color: "#8fbf9d", fontSize: 13, fontWeight: "700", textAlign: "center", lineHeight: 19, marginTop: 8 }}>
