...
// id drives behaviour, label is what the gardener reads. They used to be the same
// string, so the size rule below compared against English display text — and the
// bed's stored name was whatever English label happened to be tapped.
const GARDEN_DESIGNS = [
  { id: "vegetablePatch", small: null },
  { id: "frontYard", small: null },
  { id: "backyard", small: null },
  { id: "balcony", small: null },
  { id: "indoors", small: null },
  { id: "raisedBed", small: null },
  { id: "herbGarden", small: null },
  { id: "hangingPlanter", small: 1 },
];
const FLOWER_DESIGNS = [
  { id: "flowerBed", small: null },
  { id: "cuttingGarden", small: null },
  { id: "cottageBorder", small: null },
  { id: "windowBox", small: 3 },
  { id: "containerPot", small: 1 },
  { id: "hangingBasket", small: null },
];
...
export const GardenAreaManager = memo(function GardenAreaManager({ theme, gardenAreas, onAddArea, mode = "garden" }) {
  ...
  const designLabel = (id) => t(`gardenAreaManager.${id}`);
  const pickDesign = (d) => {
    setDesign((cur) => (cur === d.id ? null : d.id));
    // Small containers hold far less than a bed — carried on the design itself
    // rather than matched against its English name.
    if (d.small != null) setSize(d.small);
  };
  const create = () => {
    const n = (gardenAreas?.length || 0) + 1;
    const fallback = flower
      ? t("gardenAreaManager.fallbackFlowerBed", { n })
      : t("gardenAreaManager.fallbackGardenBed", { n });
    onAddArea(design ? designLabel(design) : fallback, size, flower ? "flower" : undefined);
    setDesign(null);
    setSize(6);
    close();
  };
  ...
  const label = design
    ? t("gardenAreaManager.addNamed", { count: size, design: designLabel(design) })
    : flower
    ? t("gardenAreaManager.addFlowerBedSized", { count: size })
    : t("gardenAreaManager.addBedSized", { count: size });
  ...
  return (
    <View>
      ...
      <Pressable
        ...
        accessibilityLabel={flower ? t("gardenAreaManager.addFlowerBed") : t("gardenAreaManager.addGardenBed")}
        style={({ pressed }) => [{
          flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 8,
          backgroundColor: flower ? "#ffb6c1" : "#5cff89", borderRadius: 16, paddingVertical: 16, marginTop: 4,
        }, pressed && { opacity: 0.85 }]}
      >
        <Text style={{ color: "#07120b", fontSize: 15, fontWeight: "900" }}>{flower ? t("gardenAreaManager.addFlowerBedCta") : t("gardenAreaManager.addGardenBedCta")}</Text>
      </Pressable>
        ...
        <Pressable style={{ flex: 1, backgroundColor: "rgba(0,0,0,0.6)", justifyContent: "flex-end" }} onPress={close}>
          ...
          >
            <View style={{ flexDirection: "row", alignItems: "center", marginBottom: 14 }}>
              ...
              <Pressable onPress={close} hitSlop={12} accessibilityRole="button" accessibilityLabel={t("a11y.close")}>
            ...
            <ScrollView showsVerticalScrollIndicator={false}>
              ...
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
                {DESIGNS.map((d) => {
                  ...
                  return (
                    ...
                    >
                      <Text style={{ color: active ? "#07120b" : "#8effab", fontSize: 12, fontWeight: "800" }}>{active ? "✓ " : ""}{designLabel(d.id)}</Text>
                    </Pressable>
                  );
                })}
              </View>
              ...
              <Text style={{ color: theme.secondaryText, fontSize: 11, fontWeight: "900", letterSpacing: 0.6, marginTop: 20, marginBottom: 8 }}>
                {t("gardenAreaManager.howManyPlantsCount", { count: size })}
              </Text>
              <View style={{ flexDirection: "row", flexWrap: "wrap", gap: 8 }}>
