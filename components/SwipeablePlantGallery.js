...
export const SwipeablePlantGallery = memo(function SwipeablePlantGallery({ plants, theme, zone, onOpen }) {
  ...
  if (!plants.length) return (
    <View style={styles.emptyStateCard}>
      ...
      <Text style={styles.emptyStateTitle}>{t("swipeablePlantGallery.noPlantsForYourZone")}</Text>
      <Text style={styles.emptyStateText}>
        {zone
          ? `We couldn't find plants matched to Zone ${zone} right now. Try browsing all plants on the Plants tab.`
          : t("swipeablePlantGallery.setYourZipCodeOn")}
      </Text>
    </View>
  );
  ...
  <View style={styles.galleryImageDarkWrap}>
    <Image source={imageSource} style={styles.galleryImage} resizeMode="contain" />
      ...
      <View style={styles.galleryButtonRow}>
        <Pressable onPress={() => setActiveIndex((c) => c === 0 ? plants.length - 1 : c - 1)} style={styles.galleryNavButton}><Text style={styles.galleryNavButtonText}>{t("swipeablePlantGallery.previous")}</Text></Pressable>
        <Text style={styles.galleryCounterText}>{activeIndex + 1} / {plants.length}</Text>
        <Pressable onPress={() => setActiveIndex((c) => c === plants.length - 1 ? 0 : c + 1)} style={styles.galleryNavButton}><Text style={styles.galleryNavButtonText}>{t("swipeablePlantGallery.next")}</Text></Pressable>
      </View>
    </View>
  );
})
