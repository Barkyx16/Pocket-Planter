...
export const FloatingParticle = memo(function FloatingParticle({ symbol, index, mode }) {
  // Read via the hook, not the ref: this must re-render if the user toggles the
  // OS setting, and the early return has to come after every hook has run.
  const reduced = useReducedMotion();
  const fall = useRef(new Animated.Value(0)).current;
  const drift = useRef(new Animated.Value(0)).current;
  const glow = useRef(new Animated.Value(0)).current;
  useEffect(() => {
    // Decorative continuous motion is exactly what reduced motion is for.
  ...
  }, [fall, drift, glow, index, mode, reduced]);
