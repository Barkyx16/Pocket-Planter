  ...
  if (!log.length) {
    return (
      <Text style={{ color: theme.secondaryText, fontSize: 13, fontWeight: "700", lineHeight: 19, marginTop: 2 }}>
        Log harvests (from a plant's page or the Harvest Log) and this card will track your yields and compare them season over season.
      </Text>
    );
  }
  ...
  return (
    <View>
      ...
      <View style={{ flexDirection: "row", gap: 8, marginTop: 2 }}>
        <View style={{ flex: 1, alignItems: "center", backgroundColor: "rgba(92,255,137,0.08)", borderRadius: 14, paddingVertical: 12, borderWidth: 1, borderColor: "rgba(92,255,137,0.22)" }}>
          <Text style={{ color: "#5cff89", fontSize: 22, fontWeight: "900" }}>{thisYearCount}</Text>
          <Text style={{ color: theme.secondaryText, fontSize: 10.5, fontWeight: "800", marginTop: 2 }}>Harvests in {thisYear}</Text>
        </View>
        <View style={{ flex: 1, alignItems: "center", backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 14, paddingVertical: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}>
          <Text style={{ color: theme.text, fontSize: 22, fontWeight: "900" }}>{lastYearCount}</Text>
          <Text style={{ color: theme.secondaryText, fontSize: 10.5, fontWeight: "800", marginTop: 2 }}>In {lastYear}</Text>
        </View>
      </View>

      {pct !== null ? (
        <Text style={{ color: delta >= 0 ? "#5cff89" : "#ff9f9f", fontSize: 12.5, fontWeight: "800", marginTop: 10, textAlign: "center" }}>
          {delta >= 0 ? "📈" : "📉"} {delta >= 0 ? "Up" : "Down"} {Math.abs(pct)}% vs last year ({delta >= 0 ? "+" : ""}{delta})
        </Text>
      ) : (
        <Text style={{ color: theme.secondaryText, fontSize: 12, fontWeight: "700", marginTop: 10, textAlign: "center", fontStyle: "italic" }}>
          Keep logging — next year you'll see a season-over-season comparison here.
        </Text>
      )}
      ...
      {top.length ? (
        <View style={{ marginTop: 16 }}>
          <Text style={{ color: "#8effab", fontSize: 12, fontWeight: "900", letterSpacing: 0.8, marginBottom: 10 }}>🏆 TOP PRODUCERS ({thisYear})</Text>
          <View style={{ gap: 8 }}>
            {top.map(([name, v], i) => (
              <View key={name} style={{ flexDirection: "row", alignItems: "center", gap: 10 }}>
                <Text style={{ width: 20, color: i === 0 ? "#ffd86b" : theme.secondaryText, fontSize: 13, fontWeight: "900" }}>{i + 1}</Text>
                <View style={{ flex: 1 }}>
                  <View style={{ flexDirection: "row", justifyContent: "space-between", marginBottom: 3 }}>
                    <Text style={{ color: theme.text, fontSize: 13, fontWeight: "800" }} numberOfLines={1}>{name}</Text>
