...
export const HarvestStorageGuideCard = memo(function HarvestStorageGuideCard({ theme, savedPlants, harvestLog }) {
  ...
  if (!matched.length) {
    return (
      <Text style={{ color: theme.secondaryText, fontSize: 13, fontWeight: "700", lineHeight: 19, marginTop: 2 }}>
        Save or harvest a few crops and you'll get storage tips — how to keep each one fresh and how long it lasts.
      </Text>
    );
  }
  ...
  return (
    <View>
      <Text style={{ color: theme.secondaryText, fontSize: 13, fontWeight: "700", lineHeight: 19, marginTop: 2 }}>
        Keep your harvest fresh longer — how to store what you're {harvested.length ? "picking" : "growing"}.
      </Text>
        ...
        {matched.slice(0, 10).map(({ name, g }) => {
          ...
          <View key={name} style={{ backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 14, padding: 12, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)" }}>
            <View style={{ flexDirection: "row", alignItems: "center", gap: 9 }}>
              ...
              <View style={{ backgroundColor: "rgba(92,255,137,0.12)", borderRadius: 999, paddingHorizontal: 9, paddingVertical: 3 }}>
                <Text style={{ color: "#8effab", fontSize: 11, fontWeight: "900" }}>Keeps {g.keeps}</Text>
              </View>
            </View>
            <Text style={{ color: theme.secondaryText, fontSize: 12.5, fontWeight: "700", lineHeight: 17, marginTop: 7 }}>🧊 {g.store}</Text>
            <Text style={{ color: theme.secondaryText, fontSize: 12, fontWeight: "600", lineHeight: 16, marginTop: 3 }}>💡 {g.tip}</Text>
          </View>
          );
        })}
      </View>
