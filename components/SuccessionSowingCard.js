  ...
  return (
    <View>
      <Text style={{ color: theme.secondaryText, fontSize: 13, fontWeight: "600", lineHeight: 19, marginTop: 2 }}>
        Re-sow these on a rhythm for a steady, continuous harvest instead of one big glut.
      </Text>
      ...
      <View style={{ gap: 10, marginTop: 14 }}>
        {rows.map(({ name, item, info }) => {
          ...
          return (
            <View
              key={`succ-${name}`}
              style={{ flexDirection: "row", alignItems: "center", gap: 12, backgroundColor: "rgba(255,255,255,0.04)", borderRadius: 18, padding: 13, borderWidth: 1, borderColor: `${s.color}22` }}
            >
              <View style={{ width: 46, height: 46, borderRadius: 14, backgroundColor: "#0e2414", alignItems: "center", justifyContent: "center", overflow: "hidden" }}>
                {img ? <Image source={img} style={{ width: 36, height: 36 }} resizeMode="contain" /> : <Text style={{ fontSize: 22 }}>🌱</Text>}
              </View>
              <View style={{ flex: 1 }}>
                <Text style={{ color: theme.text, fontSize: 15, fontWeight: "800" }}>{name}</Text>
                <View style={{ alignSelf: "flex-start", flexDirection: "row", alignItems: "center", backgroundColor: `${s.color}1a`, borderRadius: 999, paddingHorizontal: 9, paddingVertical: 3, borderWidth: 1, borderColor: `${s.color}33`, marginTop: 6 }}>
                  <Text style={{ color: s.color, fontSize: 10.5, fontWeight: "800" }}>{s.icon} {s.label}</Text>
                </View>
              ...
              <Pressable
                ...
                style={{ backgroundColor: actionable ? s.color : "transparent", borderRadius: 12, paddingHorizontal: 15, paddingVertical: 10, borderWidth: 1, borderColor: actionable ? s.color : `${s.color}55` }}
              >
                <Text style={{ color: actionable ? "#07120b" : s.color, fontSize: 12.5, fontWeight: "800" }}>{actionable ? "Sow" : "Sow anyway"}</Text>
              </Pressable>
            </View>
          );
        })}
