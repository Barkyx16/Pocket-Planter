...
// The canonical entry for an ISO code, or null if there is no such country.
// Case-insensitive: Android's reverse geocoder does not promise the case of
// `isoCountryCode`, and an exact-match test quietly declined to adopt "gb".
...
export function getCountry(code) {
  // Falls back to DEFAULT_COUNTRY, not COUNTRIES[0]. The first entry is
  // Afghanistan, which has no ZIP format and no bundled lookup table, so an
  // unrecognised code used to hand a US gardener a country with neither — the
  // 39,921-row offline ZIP table skipped, and the postal field relabelled.
  const entry = findCountry(code) || findCountry(DEFAULT_COUNTRY) || COUNTRIES[0];
